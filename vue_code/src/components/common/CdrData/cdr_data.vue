...
<template>
  <div>
      ...
      <template slot="body">
        <el-form :inline="true" class="mgt-10 form_search">
          <el-form-item label="数据类型">
            <el-input placeholder="请输入话单数据类型" clearable v-model="search_info.serv_type" size="mini">
      ...
      <template slot="body">
        <div class="table mgt-10">
          <el-table :data="testtableData" style="width: 100%" max-height="500px">
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-width="100px" size="mini" class="my-form">
                  ...
                  <el-form-item label="话单数据">
                    <span>{{ props.row.cdr_data_value }}</span>
                  </el-form-item>
            ...
            <el-table-column prop="serv_type" label="话单数类型" align="center"
                             :show-overflow-tooltip="true"></el-table-column>
            <el-table-column prop="key_field1" label="关键字一" align="center"
                             :show-overflow-tooltip="true"></el-table-column>
            <el-table-column prop="key_field2" label="关键字二" align="center"
                             :show-overflow-tooltip="true"></el-table-column>
            <el-table-column prop="cdr_data_value" label="话单样例数据" align="center"
                             :show-overflow-tooltip="true"></el-table-column>
            <el-table-column prop="notes" label="数据说明" align="center"
                             :show-overflow-tooltip="true"></el-table-column>
    ...
    <Dialog v-bind:visible.sync="testdialogForm" width="600" @closeDialog="testdialogForm=false" :title="dailogTitleType+'话单数据'">
      <div slot="con">
        <el-form :model="testruleForm" :rules="rules" ref="testruleForm" label-width="180px">
          <el-form-item size='mini' label="业务类型:" prop="serv_type">
            <el-input size='mini' v-model="testruleForm.serv_type" style="width: 50%"></el-input>
          </el-form-item>
          ...
          <el-form-item size='mini' label="话单样例数据:" prop="cdr_data_value">
            <el-input size='mini' v-model="testruleForm.cdr_data_value" style="width: 50%" type="textarea"></el-input>
            <el-tooltip class="item" effect="dark" placement="right-start">
              <div slot="content">多条数据以","分隔<br/>例：gg,70,0,454070013800121,,,,,,1001,,,,,</div>
              <el-button icon="el-icon-info" size="mini" type="text"></el-button>
            </el-tooltip>
          </el-form-item>
          <el-form-item size='mini' label="数据说明:" prop="notes">
            <el-input size='mini' v-model="testruleForm.notes" style="width: 50%"></el-input>
          </el-form-item>
          <el-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        rules: {
          serv_type: [{required: true, message: '话单数据类型不能为空', trigger: "change"}],
          cdr_data_value: [{required: true, message: '话单数据不能为空', trigger: 'change'}],
          notes: [{required: true, message: '数据说明不能为空', trigger: 'change'}],
        },
        // 查询输入信息
        search_info: {
          serv_type: ""
    ...
    methods: {
      ...
      reset_search() {
        this.search_info.serv_type = ''
        this.handle_search()
      },
      handle_search() {
        let data = {
          serv_type: this.search_info.serv_type,
          user_id: getCookie('user_id'),
          ser_token: getCookie('user_token')
        }
        api.config_manage_api.query_cdr_data(data).then(response => {
      ...
      handlemodify($index, row) {
        ...
        this.testruleForm.serv_type = row.serv_type
        this.testruleForm.key_field1 = row.key_field1
        this.testruleForm.key_field2 = row.key_field2
        this.testruleForm.cdr_data_value = row.cdr_data_value
        this.testruleForm.notes = row.notes
      },
      // 删除
      deleteRow(rows) {
        this.$confirm('确定删除该话单数据吗？', '确认信息', {
          ...
          let data = {
            id: rows.id,
            serv_type : rows.serv_type,
            user_id: getCookie('user_id')
          }
          api.config_manage_api.delete_cdr_data(data).then(response => {
            if (response.status === 0){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let data = {
              ...
              serv_type: this.testruleForm.serv_type,
              cdr_data_value: this.testruleForm.cdr_data_value,
              key_field1: this.testruleForm.key_field1,
              key_field2: this.testruleForm.key_field2,
              notes: this.testruleForm.notes,
              user_id: getCookie('user_id')
            }

            if (this.dailogTitleType === "编辑") {
              api.config_manage_api.update_cdr_data(data).then(response => {
                if (response.status === 0){
              ...
              api.config_manage_api.add_cdr_data(data).then(response => {
                if (response.status === 0){
      ...
      cleartestruleForm() {
        ...
        this.testruleForm.serv_type = ''
        this.testruleForm.cdr_data_value = ''
        this.testruleForm.cdr_data_type = ''
        this.testruleForm.key_field2 = ''
        this.testruleForm.key_field1 = ''
        this.testruleForm.notes = ''
      },
    },
    mounted() {
      this.handle_search()
