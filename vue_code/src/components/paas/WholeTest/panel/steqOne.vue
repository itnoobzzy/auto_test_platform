  ...
  export default {
    ...
    methods: {
      ...
      clear_host_env() {
        ...
        api.paas_whole_test_api.start_init_env_case(data).then(response => {
          this.$message.info(response.info)
        })
      },
      change_button(value) {
      ...
      get_new_add(val) {
        if (this.require_options.length === 0) {
          this.form.require_name = val.substr(0, val.length - 4)
        }
      },
      // 分页
