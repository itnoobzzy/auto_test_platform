  ...
  <div class="steq_panel_form" style="padding-bottom:50px">
    ...
    <div class="table">
      <el-table
        ...
        <el-table-column align="center" prop="expect_info" label="结果详情" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="light" placement="right-start">
              <div slot="content" style="width: 600px">
                <div class="div-a">
                  <div v-html="scope.row.really_result">{{scope.row.really_result}}</div>
                </div>
              </div>
      ...
      <div slot="con">
        ...
        <el-table :data="result_table" border :header-cell-style="{background:'#5fb3f4',color:'white'}"
            ...
            <template slot-scope="props">
              <el-form label-position="left" inline class="form_style">
                ...
                <el-form-item label="实际结果：">
                  <span v-html="props.row.real_result"></span>
                </el-form-item>
                <br>
          ...
          <el-table-column prop="task_id" width="100" label="测试任务ID" align="center"
                           :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="test_action" width="100" label="测试用例ID" align="center"
                           :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="test_name" width="120" label="测试用例名称" align="center"
                           :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="really_result" width="300" label="实际结果" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="light" placement="right-start">
                <div slot="content" style="width: 600px">
                  <div class="div-a">
    ...
    methods: {
      ...
      con_websocket(instance_id) {
        let ws = new WebSocket(
          ...
          // 'ws://' + '172.21.4.110:8299' + '/ws/query_status'
        )
        // ws.send(instance_id)
        console.log(ws)
        this.socket = ws
        ...
        let check_route_func = function() {
          ...
          if (this.test_status === '未完成' && this.$route.path === '/home/PaaS_wholeProcess'
            && this.return_first_step_flag === false && this.result_flag!==true) {
      ...
      start() {
        ...
        api.paas_whole_test_api.start_paas_special_task(data).then(res => {
          this.result_flag = true
          if (res.status === 0) {
            this.$message.success('测试任务执行完毕！')
      ...
      filter_result(result_type) {
        ...
        api.paas_whole_test_api.query_paas_result(data).then(response => {
        ...
        api.paas_whole_test_api.query_paas_result(data).then(response => {
          if (response.status === 0) {
            for (let i = 0; i < response.info.length; i++) {
              this.result_paas = response.test_pass
              this.result_not_paas = response.test_not_pass
              this.result_table = response.info
              this.result_table_count = response.total_count
            }
          } else {
      ...
      pass_is(index, rows) {
        ...
        api.paas_whole_test_api.paas_task_confirm(data).then(_ => {
      ...
      return_first_step() {
        if (this.socket !== '') {
          this.return_first_step_flag = true
          this.$confirm('剩余任务将在后台执行, 是否离开该界面?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            ...
            this.return_first_step_flag = false
            if (this.rc_loading) {
              this.$message({
                type: 'success',
                message: '稍后可到结果管理中查看执行结果!'
...
<style lang="scss" scoped>
  ...
  .three_ul {
    ...
    li {
      ...
      padding: 22px 0;
      text-align: center;
      box-shadow: 0 1px 4px 0 rgba(46, 88, 247, 0.2);
      margin-top: 49px;
      margin-bottom: 24px;
      margin-right: 16px;
